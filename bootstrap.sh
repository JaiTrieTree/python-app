...
if [[ $DEPLOY_TARGET == ec2 ]]; then
  ...
  prompt BASE_DOMAIN "test.com" "Base domain (used by ingress)"
fi
...
sudo apt-get purge -y yq || true                      # remove ancient python-yq
sudo apt-get install -y curl gnupg ca-certificates lsb-release python3 jq
...
command -v kubectl &>/dev/null || install_bin kubectl \
...
if docker info &>/dev/null; then DOCKER=docker; KIND=kind; else DOCKER='sudo docker'; KIND='sudo kind'; fi
...
EOF
helm upgrade --install argo-cd argo/argo-cd -n "$ARGO_NS" --create-namespace -f argocd-values.yaml --wait
...
kubectl -n "$ARGO_NS" wait deployment/argo-cd-argocd-server --for=condition=Available --timeout=180s
ARGO_SVC="argo-cd-argocd-server.${ARGO_NS}.svc"
argocd login "$ARGO_SVC" --insecure --username admin --password "$ARGO_PASS" --grpc-web

argocd repo add https://github.com/${GITHUB_USER}/${GITHUB_REPO}.git --username "$GITHUB_USER" --password "$GITHUB_PAT" --insecure || true
argocd app create python-app --repo https://github.com/${GITHUB_USER}/${GITHUB_REPO}.git \
  --path charts/python-app --dest-server https://kubernetes.default.svc \
  --dest-namespace "$APP_NS" --sync-policy automated --self-heal || true
argocd app sync python-app

# ------------ cert-manager & ARC ------------
log "Installing cert-manager"
kubectl apply -f https://github.com/cert-manager/cert-manager/releases/download/v1.13.1/cert-manager.yaml
kubectl -n cert-manager wait deploy/cert-manager-webhook --for=condition=Available --timeout=180s

log "Deploying ARC & runner"
helm repo add actions-runner-controller https://actions-runner-controller.github.io/actions-runner-controller >/dev/null && helm repo update >/dev/null
kubectl create namespace "$RUNNER_NS" --dry-run=client -o yaml | kubectl apply -f -
kubectl -n "$RUNNER_NS" create secret generic controller-manager --from-literal=github_token="$GITHUB_PAT" \
      --dry-run=client -o yaml | kubectl apply -f -
helm upgrade --install arc actions-runner-controller/actions-runner-controller -n "$RUNNER_NS" \
      --set syncPeriod=30s --set githubWebhookServer.enabled=false --wait
...
spec:
  template:
    spec:
      ...
      labels: ["self-hosted"]
EOF
kubectl -n "$RUNNER_NS" rollout status deployment/selfhosted-python-runner-deployment --timeout=300s || true
...
jobs:
  ...
  cd:
    ...
    steps:
      ...
      - name: Bump tag
        run: yq eval -i '.image.tag = env(IMAGE_TAG)' charts/python-app/values.yaml
      - name: Commit
        uses: EndBug/add-and-commit@v9
        with:
          message: "Update tag -> ${{ env.IMAGE_TAG }}"
      - name: Force refresh
        run: kubectl -n ARGO_NS_PLACEHOLDER annotate application/python-app argocd.argoproj.io/sync-wave="$(date +%s)" --overwrite
YAML
...
kubectl -n "$ARGO_NS" delete secret argocd-initial-admin-secret 2>/dev/null || true

cat <<EOF

=================================================================
🔑  **Add repo secrets** (GitHub → Settings → Secrets → Actions)
...
=================================================================
\033[1;32mBootstrap complete – push to ./src/ and enjoy CI/CD!\033[0m
EOF
