...
# ensure ONLY Go yq v4 is in path
sudo apt-get purge -y yq || true
sudo wget -qO /usr/local/bin/yq https://github.com/mikefarah/yq/releases/latest/download/yq_linux_amd64
sudo chmod +x /usr/local/bin/yq
log "yq version → $(yq --version)"

# wrapper for sudo-less docker/kind if group not yet active in current shell
if docker info &>/dev/null; then DOCKER=docker; KIND=kind
else warn "Using sudo docker/kind for this shell"; DOCKER='sudo docker'; KIND='sudo kind'; fi

# ───────── local kind cluster (if selected) ─────────
...
EOF
helm upgrade --install argo-cd argo/argo-cd -n "$ARGO_NS" --create-namespace \
...
sudo wget -qO /usr/local/bin/argocd \
...
argocd login "$ARGO_LOGIN_HOST" --insecure --grpc-web \
...
helm upgrade --install arc actions-runner-controller/actions-runner-controller \
     -n "$RUNNER_NS" --wait --set syncPeriod=30s --set githubWebhookServer.enabled=false

cat <<EOF | kubectl apply -f -
apiVersion: actions.summerwind.dev/v1alpha1
kind: RunnerDeployment
metadata: {name: python-runner, namespace: $RUNNER_NS}
spec:
  template:
    spec:
      ...
      labels: ["self-hosted"]
EOF
kubectl -n "$RUNNER_NS" rollout status deploy/python-runner-runner-deployment \
        --timeout=300s || true

# ───────── CI/CD workflow ─────────
log "Adding CI/CD workflow"
mkdir -p .github/workflows
cat >.github/workflows/ci-cd.yaml <<'YML'
name: ci-cd
on:
  push:
    paths: ["src/**"]
...
jobs:
  ...
  cd:
    ...
    - name: Bump image tag & commit
      run: |
        yq eval -i '.image.tag = "${{ needs.ci.outputs.commit_id }}"' charts/python-app/values.yaml
        git config user.email "bot@local" && git config user.name "bot"
        git add charts/python-app/values.yaml
        git commit -m "update image tag -> ${{ needs.ci.outputs.commit_id }}"
        git push
